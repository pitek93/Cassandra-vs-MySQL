...
def bestThriller():
	result = session.execute('SELECT title, AVG(ratings.rating) as average, COUNT(ratings.rating) as numb \
				  FROM (SELECT * FROM movies where genres like "%Thriller%") as tharray \
				  JOIN ratings on ratings.movieId = tharray.movieId \
				  GROUP BY title HAVING numb > 20 ORDER BY average DESC LIMIT 1;').fetchall()
...
def random1Mmovies(m, movieList = []):
	if(m==1):
		...
		for i in range (0,250000):
			...
			resultsql2 = session.execute('SELECT title from movies \
						      WHERE movieId = :randomMovie;', \
						    {'randomMovie':randomMovie}).fetchone()
...
def saveRandomData():
	...
	for i in range(startIndex, 160000):
		sqlAdd = session.execute('INSERT INTO movies(movieId,title,genres) \
					  VALUES(:movieId,:title,:genres);', \
					{'movieId':i, \
					 'title':"Random Title " +str(i), \
					 'genres':"Thriller|Action|Comedy|Horror"})
		for j in range(0,3):
			for k in range(1,6):
				sqlAddRating = session.execute('INSERT INTO ratings(movieId,rating) \
							        VALUES(:movieId,:rating);', \
							      {'movieId':i,'rating':k})
		session.commit()
		sqltitle = session.execute('SELECT title FROM movies \
					    WHERE movieId = :id;', \
					  {'id':i}).fetchone()
